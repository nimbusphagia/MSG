...
export async function getContactById(contactId: UuidType, currentUserId: UuidType): Promise<ContactType> {
  const contact = await prisma.contact.findUnique({
    where: { id: contactId, ownerId: currentUserId },
    include: safeUserInclude,
  });
...
export async function editNicknameServ({ id, nickname }: ContactNicknameInput, currentUserId: UuidType)
  : Promise<ContactOutput> {
  const existingContact = await prisma.contact.findUnique({ where: { id, ownerId: currentUserId } });
...
export async function deleteContactServ(id: UuidType, currentUserId: UuidType): Promise<void> {
  const existingContact = await prisma.contact.findUnique({ where: { id, ownerId: currentUserId } });
  if (!existingContact) throw new NotFoundError("Contact not found");

  await prisma.contact.delete({ where: { id } });
}
export async function toggleBlock(id: UuidType, currentUserId: UuidType): Promise<ContactOutput> {
  const existingContact = await prisma.contact.findUnique({ where: { id, ownerId: currentUserId } });
  if (!existingContact) throw new NotFoundError("Contact not found");
